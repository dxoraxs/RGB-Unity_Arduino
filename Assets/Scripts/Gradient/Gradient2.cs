...
{
    [SerializeField] private GradientMode _gradientMode = GradientMode.Global;
    [SerializeField] private GradientDir _gradientDir = GradientDir.Vertical;
    [SerializeField] private bool _overwriteAllColor = false;
    [SerializeField] private Color _vertex1 = Color.white;
    [SerializeField] private Color _vertex2 = Color.black;
    ...
    public void SetNewColor(Color color)
    {
        ...
        {
            case TypeSlider.R:
                Vertex1 = new Color(0, color.g, color.b, 255);
                Vertex2 = new Color(255, color.g, color.b, 255);
                break;
            case TypeSlider.G:
                Vertex1 = new Color(color.r, 0, color.b, 255);
                Vertex2 = new Color(color.r, 255, color.b, 255);
                break;
            case TypeSlider.B:
                Vertex1 = new Color(color.r, color.g, 0, 255);
                Vertex2 = new Color(color.r, color.g, 255, 255);
                break;
        }
    ...
    public override void ModifyMesh(VertexHelper vh)
    {
        ...
        if (_gradientMode == GradientMode.Global)
        {
            ...
            float bottomY = _gradientDir == GradientDir.Vertical
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                uiVertex.color *= Color.Lerp(_vertex2, _vertex1,
                    ((_gradientDir == GradientDir.Vertical ? uiVertex.position.y : uiVertex.position.x) - bottomY) /
                    uiElementHeight);
                ...
                switch (_gradientDir)
                {
                    ...
                    case GradientDir.DiagonalLeftToRight:
                        uiVertex.color *= (i % 4 == 0)
                            ? _vertex1
                            : ((i - 2) % 4 == 0 ? _vertex2 : Color.Lerp(_vertex2, _vertex1, 0.5f));
                        break;
                    case GradientDir.DiagonalRightToLeft:
                        uiVertex.color *= ((i - 1) % 4 == 0)
                            ? _vertex1
                            : ((i - 3) % 4 == 0 ? _vertex2 : Color.Lerp(_vertex2, _vertex1, 0.5f));
                        break;
                }
