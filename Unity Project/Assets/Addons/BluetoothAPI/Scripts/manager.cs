...
public class manager : MonoBehaviour {
	...
	void Start () {
		deviceName = "JDY-31-SPP"; //bluetooth should be turned ON;
		try
		{	
			...
			*/
            ...
            NativeUI.ShowToast(ds.ToString());
	...
	void OnMessageReceived()
	{
        ...
        received_message = bluetoothHelper.Read();
        NativeUI.ShowToast(received_message);
    ...
    private void ScanEnded2(LinkedList<BluetoothDevice> devices){
	    NativeUI.ShowToast(devices.Count.ToString());
    }
	...
	void OnGUI()
	{
		...
		{
			var data = lastClick ? "1" : "0";
			bluetoothHelper.SendData(data);
			NativeUI.ShowToast($"Send data = {data}");
			lastClick = !lastClick;
